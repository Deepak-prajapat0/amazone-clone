...
export default function ProductDetails() {
  ...
  return (
    <Box p="4" display="flex" flexDirection={{ base: "column", md: "row" }} flexWrap="nowrap" justifyItems="center">
      {/* image box */}
      <Box maxWidth="30rem" display="flex" flexDirection={{ base: "column", md: "row" }} alignItems={{ base: "center", md: "start" }} gap="5px" position="sticky" top="0" left="0">
        <Box display="flex" gap="6px" flexDirection={{ base: "row", md: "column" }} minWidth="4rem">
          {data.image_url.map((url: string, index: number) =>
            <Image key={index} src={url} onClick={() => setImage(index)} alt="multiple photos" height="5rem" width="100%" cursor="pointer" />
          )}
        </Box>
      ...
      {/* detailbox */}
      <VStack maxWidth="35rem" alignItems="start" gap="1" px="4" fontSize={24}>
      ...
      {/* button box */}
      <Show above="md">
        <VStack width="fit-content" h="max-content" p="4" border="1px solid lightgray" borderRadius="8">
          <Text fontSize="14" fontWeight="bold"><span style={{ color: "#307AC6" }}>Free delivery</span> Monday, 28 October.Order within 19hrs.</Text>
          <Button size="sm" h="9" width="80%" borderRadius="16" bg="#FFD814" fontWeight="light" >Add to Cart</Button>
